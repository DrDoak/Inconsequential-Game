...
function ModInventory:checkInt(fixture)
	if fixture then
		...
		if other and Class.istype(other, "ObjInteractive") and other.intBoxActive then
			if not Class.istype(other, "ObjEquippable") and other ~= self.currentEquips then
...
function ModInventory:drop(slot,removeFromInv,destroy)
	local r, c
	if not self.currentEquips[slot] then return end

	if self.currentEquips[slot].sprClass then 
		r, c = self.inventory:getRC(self.currentEquips[slot].sprClass)
...
function ModInventory:addToInv(item, stackable, amount,overwrite, animate)
	...
	self:equipIfOpen(item )
	self.inventory:insert( item )
	self:updateIntAmount()
end

function ModInventory:equipIfOpen(item)
	-- lume.trace()
