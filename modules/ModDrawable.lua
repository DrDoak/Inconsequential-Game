...
function ModDrawable:create()
	...
	self.vHeight = self.vHeight or self.height
...
function ModDrawable:addShadow( shape,width,height )
	local drawFunc
	if shape == "rect" then
		drawFunc = function()
			local loveGraphics = love.graphics
			loveGraphics.push()
			-- loveGraphics.translate( self.x, self.y + 16 )
			loveGraphics.setColor(20, 20, 20,150)
			love.graphics.rectangle( "fill", self.x,self.y + 16 , width,height )
		...
		drawFunc = function()
			...
			loveGraphics.push()
			loveGraphics.setColor(20, 20, 20,150)
			loveGraphics.ellipse( "fill", self.x, self.y + 16, width/2, height/2 )
			loveGraphics.setColor(255, 255, 255)
			loveGraphics.pop()
	    end
	end
	self.shadow = Scene.wrapNode({draw = drawFunc}, 0)
	Game.scene:insert(self.shadow)
end
