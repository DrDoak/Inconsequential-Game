local ObjCamera = Class.create("ObjCamera", Entity)
...
ObjCamera.instanceCount = 0
local function addinst(count)
	ObjCamera.instanceCount = ObjCamera.instanceCount + count
end

function ObjCamera:create()
	local mprops = Game.map.properties
	local map = Game.map
	if ObjCamera.instanceCount > 0 then
		Game:del(self)
	else
		addinst(1)
	end
...
function ObjCamera:destroy()
	addinst(-1)
end

function ObjCamera:tick(dt)
	local mprops = Game.map.properties
	self:itrack()
	self:iShake()
end

function ObjCamera:setOffset( x,y )
	self.offsetx = x
	self.offsety = y
end

function ObjCamera:setTrack( ObjUnit )
	self.target = ObjUnit or Game.player
end

function ObjCamera:itrack()
	local player
...
function ObjCamera:setShake( duration, intensity, speed )
